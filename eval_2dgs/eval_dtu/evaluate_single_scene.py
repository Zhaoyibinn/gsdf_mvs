...
def cull_scan(scan, mesh_path, result_mesh_file, instance_dir):
    ...
    if os.path.isdir(mask_dir):
        ...
        mask_dir = '{0}/masks'.format(instance_dir)
        mask_paths = sorted(glob.glob(os.path.join(mask_dir, "*.png")))
    masks = []
    for p in mask_paths:
        mask = cv2.imread(p)
...
if __name__ == "__main__":
    ...
    cmd = f"/home/zhaoyibin/anaconda3/envs/surfel_splatting/bin/python {script_dir}/eval.py --data {result_mesh_file} --scan {scan} --mode mesh --dataset_dir {Offical_DTU_Dataset} --vis_out_dir {out_dir}"
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print(cmd)
    os.system(cmd)